  ...
  return (
    <div className={classes.event}>
      ...
      {events.map((e) => {
        ...
        if (e.hall_id != null)
          allProviderIdInEvent.push({ id: e.hall_id, hall_name: e.hall_name });
        e.chiefs.forEach((chief) => {
          allProviderIdInEvent.push({
            id: chief.id,
            chief_name: chief.name,
          });
        });

        // --- לוגיקה לבדיקה אם האירוע בעתיד ---6
        const isFuture =
              ...
              {rolePath === "customer" &&
                ...
                e.finalStatus === "APPROVED" && (
                  <>
                    ...
                    {allProviderIdInEvent.map((p) => (
                      <div>
                        <Review key={p.id} provider={p} eventId={e.event_id} />
                      </div>
                    ))}
                  </>
                )}
