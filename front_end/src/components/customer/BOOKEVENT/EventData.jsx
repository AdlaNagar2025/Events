...
{chiefsData.map((chief) => {
  const chefKey = chief.chief_id || chief.id;

  const existingChefData = Array.isArray(dataToEvent?.chiefs)
    ? dataToEvent.chiefs.find((c) => (c.chief_id || c.id) == chefKey)
    : null;

  const currentNote =
    noteToChef?.[chefKey] ??
    noteToChef?.[String(chefKey)] ??
    existingChefData?.noteToChef ??
    "";

  return (
    <div key={chefKey} className={classes.chefCard}>
      ...
      <div className={classes.inputGroup}>
        <textarea
          value={currentNote}
          onChange={(e) =>
            handleChefNoteChange(chefKey, e.target.value)
          }
